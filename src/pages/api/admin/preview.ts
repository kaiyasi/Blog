...
function previewDocument(payload: Record<string, unknown>, body: string) {
  ...
  const tags = Array.isArray(payload.tags)
    ...
    : '';
  const kind = payload.kind === 'project' ? 'PROJECT' : 'ARTICLE';
  return `<!doctype html>
...
</style></head><body><main class="page"><header><p class="kicker">${kind} PREVIEW</p><h1 class="title">${title}</h1><p class="description">${description}</p><div class="meta"><time>${date}</time>${tags}</div></header><article class="prose">${body || '<p class="empty">開始輸入 Markdown 後，預覽會顯示在這裡。</p>'}</article></main></body></html>`;
}
