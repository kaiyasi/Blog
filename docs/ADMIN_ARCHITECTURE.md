...
- 後台「關於」分頁使用固定欄位與可增減清單，不直接暴露 JSON；格式或必要欄位不正確時不會覆寫原檔。
- About Me、Roadmap、專案說明與經歷說明以 Markdown 編輯，公開頁會渲染 Markdown；個人資料與網址使用明確欄位。
- 後台只維護繁中副標題與簡介；儲存時由 `AI_TRANSLATION_MODEL` 自動更新英文、日文與韓文版本。
